(function ($, window, document)
{
    ...
    $.imageFocus.focusPoint = function (el, options)
    {
        ...
        base.addInterfaceElements = function ()
        {
            ...
            //var $thumbnail = $('.edit-attachment-frame .attachment-media-view .details-image');
        ...
        base.attachment = {
            ...
            getData: function ()
            {
                base.attachment._id = document.querySelector('.wpmf_attachment_id')?.value;
        ...
        base.sendImageCropDataByAjax = function (createNewThumb)
        {

            if (createNewThumb === false) {
                createNewThumb = false;
            } else {
                createNewThumb = true;
            }

            var prepData = {
                id: base.attachment._id || document.querySelector('.wpmf_attachment_id')?.value,
                focusPoint: base.attachment._focusPoint,
                doCrop: createNewThumb
            };
...
(function ($, window, document)
{
    ...
    $(document).on('ready', function ()
    {
        setInterval(function ()
        {
            ...
            if ($attachmentDetails.length <= 0 && $('body').hasClass('post-type-attachment') && $('body').hasClass('post-php')) {
                $attachmentDetails = $('.wp_attachment_holder');
            }

            var $detailImage = getImage();
...
/**
 * Get image, to where focus point functionality will be added.
 *
 * @returns {*|jQuery|HTMLElement}
 */
function getImage() {
    // Media lib / Media popup
    var $image = jQuery('.media-frame-content .attachment-details .thumbnail img');

    if( $image.length <= 0 && jQuery('body').hasClass('post-type-attachment') && jQuery('body').hasClass('post-php') ) {
        $image = jQuery('.wp_attachment_holder .wp_attachment_image img.thumbnail');
    }
